...
void runDiamond(string seqType, string subFile, string dbFile, string simToolOptions, unsigned int l)
{
	...
	string simCmd;
        if(seqType == "prot")
        {
                simCmd = diamondDir + "/diamond blastp --query " + subFile + " --db " + dbFile + ".dmnd " + simToolOptions + " --out " + subFile + ".out --quiet";
        }
        else if(seqType == "nucl")
        {
                simCmd = diamondDir + "/diamond blastn --query " + subFile + " --db " + dbFile + ".dmnd " + simToolOptions + " --out " + subFile + ".out --quiet";
        }
        system((simCmd).c_str());
}
...
void runSimilarity( string fileIn, string simTool, string seqType, string configDir, string outDir, unsigned int nCpu, string timeInfo)
{
	...
	if(simTool == "blast")
	{
		if(seqType == "prot")
		{
			...
			// Create db
			system((blastDir + "/makeblastdb -in " + fileIn + " -dbtype prot -logfile " + fileIn + ".makeblastdb.log").c_str());
		}
		else if(seqType == "nucl")
		{
			// Get config file
			...
			// Create db
			system((blastDir + "/makeblastdb -in " + fileIn + " -dbtype nucl -logfile " + fileIn + ".makeblastdb.log").c_str());
		}
	}
	else if(simTool == "diamond")
	{
		...
		// Create db
		system((diamondDir + "/diamond makedb --in " + fileIn + " --db " + fileIn + " --quiet").c_str());
	}
